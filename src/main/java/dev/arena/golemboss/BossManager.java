...
public class BossManager implements Listener {
    ...
    private static class ActiveBoss {
        ...
        BossBar bar;
        BukkitTask barTask, particleTask, minionTask, aggroTask, attackTask;
        long lastBossAttack = 0;
    ...
    private void startTasks(ActiveBoss b) {
        ...
        // Force boss to actually swing at target (vanilla IronGolem AI sometimes doesn't trigger reliably for custom entities)
        if (b.attackTask == null) {
            b.attackTask = plugin.getServer().getScheduler().runTaskTimer(plugin, () -> {
                if (b.defeated || b.entity == null || b.entity.isDead() || !b.entity.isValid()) return;
                LivingEntity tgt = b.entity.getTarget();
                if (tgt == null || tgt.isDead() || !tgt.isValid() || !(tgt instanceof Player)) return;
                Location bossLoc = b.entity.getLocation();
                Location tgtLoc = tgt.getLocation();
                if (bossLoc.getWorld() != tgtLoc.getWorld()) return;
                double dist = bossLoc.distance(tgtLoc);
                if (dist > 4.0) return; // only melee range
                // swing animation + deal damage via our handler
                b.entity.swingMainHand();
                // trigger damage to player (our listener will handle the rest)
                if (System.currentTimeMillis() - b.lastBossAttack > 800) {
                    b.lastBossAttack = System.currentTimeMillis();
                    tgt.damage(BOSS_DMG, b.entity); // this will go to onBossOrMinionAttacksPlayer
                }
            }, 20L, 10L);
        }
    }
    ...
    private void spawnMinions(ActiveBoss b, Player attacker) {
        ...
        for (int i = 0; i < MINIONS; i++) {
            ...
            Player t = attacker != null ? attacker : nearestPlayer(m, MINION_AGGRO);
    ...
    private void startPhase2(ActiveBoss b) {
        ...
        startTasks(b);  // ensure aggro + attack tasks run for new entity
        startPhase2Tasks(b);
        spawnMinions(b, null);
        updateBossBar(b);
    ...
    private void stopTasks(ActiveBoss b) {
        ...
        if (b.attackTask != null) b.attackTask.cancel();
    }
    ...
    /** Отменяем не-игровой урон по боссу (падение, огонь и т.д.), чтобы HP вёл только плагин. */
    @EventHandler(priority = EventPriority.LOWEST)
    public void onAnyBossDamage(EntityDamageEvent e) {
        if (!isBoss(e.getEntity())) return;
        // Не отменяем EntityDamageByEntityEvent здесь — его обработаем ниже,
        // чтобы события от игроков доходили.
    ...
    public void onBossHitByPlayer(EntityDamageByEntityEvent e) {
        ...
        if (dmg <= 0) dmg = 1.0;
        e.setCancelled(true); // гасим ванильный урон, применяем свой
